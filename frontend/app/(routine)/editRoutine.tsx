...
type ItemProps = { productType: string, productName: string, morningRoutine: boolean, eveningRoutine: boolean }
const Item = ({ productType, productName, morningRoutine, eveningRoutine }: ItemProps) => (
        ...
        <View style={styles.container}>
            <Modal visible={openModal} onRequestClose={() => { setOpenModal(false) }} animationType='slide' transparent={true}>
                <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.5)' }}>
                    <View style={{ width: 350, height: 350, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 20 }}>
                        <Text style={{ fontWeight: '700', marginBottom: 5 }}>Add a product</Text>

                        <Text>Product Type</Text>
                        <View style={styles.dropdownContainer}>
                            <DropdownComponent options={productTypeOptions} onSelect={handleSelectProductType} />
                        </View>
                        ...
                        <CircleCheckbox label="Morning routine" checked={morningRoutine} onChange={setMorningRoutine} />
                        <CircleCheckbox label="Evening routine" checked={eveningRoutine} onChange={setEveningRoutine} />

                        <Text>Frequency</Text>
                        <View style={{display:'flex', flexDirection:'row', marginBottom: 10}}>
                            <FrequencyCheckbox label="M" checked={selectedDays.includes('Monday')} onChange={() => toggleDay('Monday')} />
                            <FrequencyCheckbox label="T" checked={selectedDays.includes('Tuesday')} onChange={() => toggleDay('Tuesday')} />
                            <FrequencyCheckbox label="W" checked={selectedDays.includes('Wednesday')} onChange={() => toggleDay('Wednesday')} />
                            <FrequencyCheckbox label="T" checked={selectedDays.includes('Thursday')} onChange={() => toggleDay('Thursday')} />
                            <FrequencyCheckbox label="F" checked={selectedDays.includes('Friday')} onChange={() => toggleDay('Friday')} />
                            <FrequencyCheckbox label="S" checked={selectedDays.includes('Saturday')} onChange={() => toggleDay('Saturday')} />
            ...
            <FlatList
                data={data}
                renderItem={({ item }) =>
                    <Item productType={item.productType} productName={item.productName} morningRoutine={item.morningRoutine} eveningRoutine={item.eveningRoutine} />}
...
const data = [
    ...
    },
    {
        ...
        morningRoutine: true,
        eveningRoutine: true,
        frequency: { 'Monday': true, 'Tuesday': false, 'Wednesday': false, 'Thursday': true, 'Friday': true, 'Saturday': true, 'Sunday': true }
    }
]
